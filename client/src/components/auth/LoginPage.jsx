const Login = () => {
  return (
    <div className=" flex ">
      <div className="min-w-[938px] h-screen bg-white grid place-content-center ">
        <div className=" text-[64px] leading-[96px] font-bold text-center">
          Login To Your Account
        </div>
        <div className="text-[32px] leading-[48px] font-normal  text-center">
          Login using social networks{" "}
        </div>
        <div className="flex justify-center">
          <a href="">
            <img
              className="w-[60px] mr-4"
              src="https://img.freepik.com/premium-vector/round-facebook-logo-isolated-white-background_469489-897.jpg"
              alt=""
            />
          </a>
          <a href="">
            <img
              className="w-[65px] mr-4"
              src="https://cdn-teams-slug.flaticon.com/google.jpg"
              alt=""
            />
          </a>
          <a href="">
            <img
              className="w-[65px] mr-4"
              src="data:image/png;base64,"
              alt=""
            />
          </a>
        </div>

        <hr className="h-px my-8 bg-gray-200 border-0 dark:bg-gray-700"></hr>
        <div className="grid place-content-center">
          <div>
            <input
              placeholder="Email"
              className=" p-3 w-[611px] h-[77px] bg-[#ECF5F3] rounded-3xl text-[20px] text-normal"
            ></input>
          </div>
          <div>
            <input
              placeholder="Password"
              className=" p-3 mt-5 w-[611px] h-[77px] bg-[#ECF5F3] rounded-3xl text-[20px] text-normal"
            ></input>
          </div>

          <a href="" className=" text-right text-normal text-[20px] ">
            Forgot Password?
          </a>
        </div>

        <div className="flex justify-center mt-6 ">
          <button className=" hover:bg-[#3ebca3ae] w-[322px] h-[77px] text-center rounded-[30px] bg-[#28B498] text-white text-[32px] text-normal">
            Sign In
          </button>
        </div>
      </div>
      <div className="min-w-[576px] h-screen bg-gradient-to-r from-[#3FAAA8F2] to-[#39B792] flex justify-center">
        <div className="w-[498px] h-screen grid place-content-center">
          <div className=" text-center text-white font-bold text-[64px]">
            New Here?
          </div>

          <div className=" text-center  text-white font-medium text-[32px]">
            Sign up and discover a great amount of new opportunities!
          </div>
          <div className=" flex justify-center">
            <button className="mt-10 w-[322px] h-[77px] bg-white rounded-[30px] text-center text-[32px] font-semibold text-gray-500">
              Sign Up
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Login;
